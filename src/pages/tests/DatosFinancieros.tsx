...
let validationSchema =  Yup.object<IfinVal>({
    numero_voucher: Yup.string().required(msgReq),
...
export default function DatosFinancieros({onSubmit, handleBack, activeStep}:Props) 
{
    ...
    if(data.trabajador){
        myData = [
            {value:(precio - precio*0.8).toString(), label:`S/${(precio - precio*0.8).toFixed(2)} - presentar certificado de trabajo(docente)`},
